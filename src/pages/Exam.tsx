	...
	const startExam = async () => {
		try {
			...
			if (!data || data.length < TOTAL_QUESTIONS) {
				toast({
					title: "Za mało pytań",
					description: `W wybranej kategorii jest tylko ${
						data?.length || 0
					} pytań. Potrzebne minimum ${TOTAL_QUESTIONS}.`,
					variant: "destructive",
				});
				return;
			}
				...
				<Card>
					...
					<CardContent className="space-y-6">
						...
						<div className="p-4 bg-accent/10 border border-accent/20 rounded-lg">
							<p className="text-sm">
								<strong>Ważne:</strong> Po rozpoczęciu egzaminu nie będziesz
								mógł go przerwać. Timer będzie odliczał czas automatycznie.
							</p>
						</div>
				...
				<CardContent className="space-y-3">
					{["A", "B", "C", "D"].map((key) => {
						const answerText = currentQuestion[
							`answer_${key.toLowerCase()}` as keyof Question
						] as string;
