...
const MetarQuiz = () => {
  ...
  const generateQuestions = (data: MetarData) => {
    ...
    allQuestions.push({
      ...
      options: [windSpeed, windSpeed + 5, Math.max(0, windSpeed - 3), windSpeed + 10]
    ...
    allQuestions.push({
      ...
      correct: `${visKm} km`,
      options: [visibility, visibility + 2, Math.max(0.5, visibility - 1.5), Math.min(10, visibility + 3)]
        .sort(() => Math.random() - 0.5)
    ...
    allQuestions.push({
      ...
      options: ["Tak", "Nie", "Tylko w dzień", "Tylko w nocy"].sort(() => Math.random() - 0.5),
    ...
    allQuestions.push({
      ...
      correct: windSpeed < 10 ? "Słaby" : windSpeed < 20 ? "Umiarkowany" : "Silny",
      options: ["Słaby", "Umiarkowany", "Silny", "Bardzo silny"].sort(() => Math.random() - 0.5),
    ...
    allQuestions.push({
      ...
      options: ["Tak", "Nie", "Tylko z instruktorem", "Zależy od doświadczenia"].sort(() => Math.random() - 0.5),
    ...
    allQuestions.push({
      ...
      correct: "Co godzinę",
      options: ["Co 15 minut", "Co godzinę", "Raz dziennie", "Co 6 godzin"].sort(() => Math.random() - 0.5),
    ...
    allQuestions.push({
      ...
      correct: "8 km",
      options: ["5 km", "8 km", "10 km", "15 km"].sort(() => Math.random() - 0.5),
    ...
    allQuestions.push({
      ...
      correct: "Węzły (knots)",
      options: ["Węzły (knots)", "Kilometry", "Kierunek wiatru", "Temperatura"].sort(() => Math.random() - 0.5),
      ...
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-background via-background to-primary/5 p-4">
        <Card className="w-full max-w-3xl shadow-lg">
          <CardHeader className="text-center space-y-4">
            <div className={`mx-auto w-20 h-20 rounded-full flex items-center justify-center text-3xl font-bold ${
              score >= 70 ? 'bg-green-100 text-green-600' : 'bg-orange-100 text-orange-600'
            }`}>
              {score}%
            </div>
            <CardTitle className="text-3xl">
              {score >= 90 ? '🎉 Doskonale!' : score >= 70 ? '✅ Świetnie!' : '📚 Jeszcze trochę nauki'}
            </CardTitle>
          ...
          <CardContent className="space-y-4">
            {metarData && (
              <div className="bg-muted/50 p-4 rounded-lg border">
                <p className="text-xs text-muted-foreground mb-2 font-semibold">METAR {icao}:</p>
                <p className="font-mono text-sm break-all leading-relaxed">{metarData.raw}</p>
              </div>
            )}
              ...
              {answers.map((answer, idx) => (
                <div key={idx} className={`p-3 rounded-lg border-l-4 ${
                  ...
                  <p className="text-sm mt-1">
                    {answer.isCorrect ? '✅' : '❌'} Twoja odpowiedź: <span className="font-semibold">{answer.selectedAnswer}</span>
                  </p>
                  {!answer.isCorrect && (
                    <p className="text-sm mt-1 text-green-700">
                      ✓ Prawidłowa: <span className="font-semibold">{questions[idx].correct}</span>
                    </p>
                  )}
                </div>
              ))}
            ...
            <div className="flex gap-3 pt-4">
              ...
              <Button onClick={refreshQuestions} variant="outline" className="flex-1" size="lg">
                <RefreshCw className="mr-2 h-4 w-4" />
                Inne pytania
              </Button>
            </div>
        ...
        <CardHeader>
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-medium text-muted-foreground">Lotnisko</p>
          ...
          <CardDescription>
            <div className="bg-muted/50 p-4 rounded-lg border">
              <p className="text-xs text-muted-foreground mb-2 font-semibold">METAR:</p>
              <p className="font-mono text-xs break-all leading-relaxed">{metarData.raw}</p>
            </div>
          </CardDescription>
        </CardHeader>
            ...
            <div className="space-y-4">
              <div className={`p-6 rounded-lg text-center border-2 ${
                ...
                <p className="text-2xl font-bold mb-2">
                  {answerFeedback.correct ? '✅ Świetnie!' : '❌ Niestety, to nie ta odpowiedź'}
                </p>
                {!answerFeedback.correct && (
                  <>
                    ...
                    <div className="bg-white/50 p-4 rounded-lg border border-blue-200">
                      <p className="text-sm text-left leading-relaxed">
                        💡 <span className="font-semibold">Wyjaśnienie:</span> {currentQ.explanation}
                      </p>
                    </div>
                  </>
                )}
